...
<template>
  ...
  <main class="account-page dense-page">
    ...
    <section class="panel"><div class="section-head"><div><h2>交易图表</h2><small>数据库生成 · 持久化任务</small></div><a href="http://127.0.0.1:8866/" target="_blank">打开 K 线任务中心</a></div><div class="kline-controls"><label>开始时间<input v-model="klineForm.start" type="datetime-local"></label><label>结束时间<input v-model="klineForm.end" type="datetime-local"></label><button class="primary" :disabled="klineJob && ['queued','running'].includes(klineJob.status)" @click="startKline">生成 K 线图</button><button v-if="klineJob && ['queued','running'].includes(klineJob.status)" @click="cancelJob(klineJob)">取消</button></div><div v-if="klineJob" class="job-card compact-job"><div class="job-head"><b>{{ jobMessage(klineJob) }}</b><span>{{ klineJob.progress || 0 }}%</span></div><div class="progress"><i :style="{width:`${klineJob.progress || 0}%`}" /></div><div v-if="klineJob.error" class="inline-error">{{ klineJob.error }}</div></div><div class="chart-list"><article v-for="chart in detail.data.value?.charts || []" :key="chart.name"><div><b>{{ chart.name }}</b><span>{{ chart.start || '-' }} 至 {{ chart.end || '-' }} · {{ chart.sizeText }}</span></div><div><a :href="chart.url" target="_blank">预览</a><a :href="chart.url" target="_blank">打开</a></div></article><span v-if="!detail.data.value?.charts?.length">当前开发产物目录暂无图表。</span></div></section>
