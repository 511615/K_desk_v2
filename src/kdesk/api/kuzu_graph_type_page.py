...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}body{margin:0;min-height:100vh;background:radial-gradient(circle at 50% -10%,#123f72,transparent 45%),#061326}
    ...
    @media(max-width:1100px){.grid{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(max-width:760px){.grid{grid-template-columns:1fr}.head{align-items:flex-start;flex-direction:column}}
  </style>
</head>
<body>
<main data-graph-type-selector="true">
  ...
  <div class="grid">
    ...
    <section class="card"><h2>固定区域关系网</h2><p>问题账户和同名账户位于中心，IP、CID、EA、跟单、返佣、IB/CRM、同步和对锁关系各在固定扇区内展示。</p><div class="preview force"><i class="dot"></i><i class="peer p1"></i><i class="peer p2"></i><i class="peer p3"></i></div><a class="choose" data-type="fixed-sector" href="?graph_type=fixed-sector">使用固定区域关系网</a></section>
