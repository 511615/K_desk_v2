...
def build_kline_timeline(replay: dict[str, Any], *, start: str = "", end: str = "") -> dict[str, Any]:
    ...
    # This is an account ledger, not a trade summary.  Preserve every opening
    # and closing entry so each row reflects the Balance/Credit after that action.
    selected_events = [
        _public_event(item)
    ...
    summary.update(
        {
            ...
            "fundsEventCount": sum(item.get("category") == "funds" for item in selected_events),
            "orderEventCount": sum(item.get("category") == "order" for item in selected_events),
