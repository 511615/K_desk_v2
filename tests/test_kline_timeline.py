...
def test_kline_timeline_keeps_known_carry_in_and_preserves_open_close_cashflow_events() -> None:
    replay = {
    ...
    assert timeline["events"][0]["orderId"] == "101"
    assert timeline["events"][2]["deltaBalance"] == -125.0
    assert timeline["summary"]["orderEventCount"] == 2
...
def test_kline_timeline_keeps_unknown_pre_anchor_state_unknown_instead_of_using_defaults() -> None:
    ...
    assert timeline["events"][0]["kind"] == "trade_open"
    assert timeline["events"][0]["category"] == "order"
