...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; }
    ...
    .account-link:hover,.account-link:focus-visible { color:#fff; background:var(--accent); border-color:var(--accent); }
    .inline-kline-frame { display:block; width:100%; min-height:620px; border:0; overflow:hidden; background:#061322; }
...
<body>
  ...
  <main>
    ...
    <section class="section" id="inlineKlineSection" hidden>
      ...
      <iframe class="inline-kline-frame" id="inlineKlineFrame" title="账户交易 K 线" sandbox="allow-scripts" scrolling="no"></iframe>
  ...
  <script>
    ...
    function applyInlineKlineHeight(event){const frame=$("inlineKlineFrame"),message=event.data;if(event.source!==frame.contentWindow||!message||message.type!=="kdesk-inline-kline-height")return;const height=Number(message.height);if(!Number.isFinite(height))return;frame.style.height=`${Math.max(620,Math.min(50000,Math.ceil(height)))}px`;}
    window.addEventListener('message',applyInlineKlineHeight);
    function inlineKlineDocument(html){return html.replace('<head>',`<head><base href="${location.origin}/">`);}
    async function loadInlineKline(){const db=state.detail?.database||{},source=db.latestSource||{},section=$("inlineKlineSection"),status=$("inlineKlineStatus"),frame=$("inlineKlineFrame");if(!source.platform||!source.server){section.hidden=true;return;}const current=filters(),query=new URLSearchParams({platform:current.platform||source.platform,server:current.server||source.server,recentOrders:'300'});query.set('inlineVersion',Date.now().toString());const key=`${query.toString()}|${db.lastTime||db.orderCount||0}`;section.hidden=false;if(state.inlineKlineKey===key)return;state.inlineKlineKey=key;status.textContent='正在读取最近 300 笔订单与当前持仓及 M1 报价...';frame.style.height='620px';frame.removeAttribute('srcdoc');try{const response=await fetch(`/api/accounts/by-login/${encodeURIComponent(LOGIN)}/inline-kline?${query}`,{cache:'no-store'});if(!response.ok){let issue={};try{issue=await response.json();}catch(_){ }throw new Error(issue.error||issue.detail||`HTTP ${response.status}`);}frame.srcdoc=inlineKlineDocument(await response.text());status.textContent='最近 300 笔订单与当前持仓 · 已直接加载';}catch(err){frame.srcdoc='';if(String(err.message||'').includes('当前持仓')){section.hidden=true;return;}status.textContent=`K 线暂时无法加载：${err.message}`;}}
